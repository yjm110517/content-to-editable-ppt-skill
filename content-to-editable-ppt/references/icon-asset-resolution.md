...
Resolve `role=icon` placeholders offline from pinned Tabler Outline 3.46.0. Unique exact canonical names and unique official aliases may be selected automatically. Every other outcome must be saved as an immutable Host Selection Decision in the current Host pass: either select one current Top-K candidate or explicitly record `selected_icon = null` with `no_accurate_tabler_match`. Do not add another Host pass, call an independent Icon Reviewer, or use an online service.
...
Do not call the historical two-Tabler composition or programmatic-SVG paths. Create Raster Handoff Pending only from a current Authority-bound Search Evidence and immutable Host Decision; it is not SVG resolution success and must not create a fake Resolution Record, Asset Manifest, Security Report, or Consumption Contract. Extraction occurs only after the current Design Preview is approved; it must bind the Preview hash, `visual_ref`, crop box, PNG hash, background-removal status, and extraction quality. Reject extraction that includes formal text, unrelated elements, the whole slide, insufficient resolution, severe background fusion, or occlusion.
