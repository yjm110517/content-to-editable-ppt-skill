...
本阶段先在旧实现后建立一个最小生产入口，完成真实 Deck 验证后，再在同一切换提交中启用新入口并重写 `SKILL.md`、README。不得出现文档已切换但代码尚不可用的中间状态。
...
6. 使测试、Smoke、Fixture 和阶段评估器无法从生产入口到达。
...
建议使用 `scripts/run.py`，实际名称可在阶段 1 决定。它是唯一外部生产命令，不提供用户必须理解的 `plan/build/verify/deliver` 子命令。
...
- 通过同一个外部入口的明确请求类型访问；
- 只保留其必要 Planner 或底层模块；
- 不让其状态机、Reviewer 或 Evidence 进入多页内容路径；
- 为该入口保留一套最小 Fixture。

如果决定删除，则本阶段不实现任何兼容逻辑。
...
新用户只通过 `SKILL.md` 能理解：提供材料、确认方案、得到 PPTX。宿主只发现一个生产入口，且该入口已通过真实多页任务。
