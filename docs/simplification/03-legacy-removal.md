...
- 状态：依赖阶段 2
- 前置文档：[阶段 2：建立并原子切换唯一入口](02-single-entry-cutover.md)
...
本阶段执行真正的文件精简。只能删除已从唯一生产入口断开的功能簇。历史恢复依赖 Git，不创建新的 `archive/`、`legacy-backup/` 或迁移数据库。
...
7. 保持唯一入口、核心编辑性和 PowerPoint 运行能力不回退。
...
如果删除导致唯一入口失败，应恢复该功能簇并重新研究依赖。不得新增：
...
如果唯一入口仍 import 某个 `manage_*` 文件，应先把必要函数迁移到 `scripts/core/` 或等价内部模块，再删除外部 CLI 包装。
...
- 保留兼容入口所需的最小 Planner 时，将其限制在该入口；
...
按功能簇回退。恢复失败功能簇后重新执行唯一入口验证。不得整体恢复 P0～P5，也不得用长期适配器代替正确依赖分析。
