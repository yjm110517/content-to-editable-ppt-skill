# Content to Editable PPT Skill 单页 Runtime 执行与错误恢复规范 v1.0

> 英文名：Single-Slide Runtime Execution & Error Recovery Specification v1.0
...
且具备当前任务所需能力时，才能进入 Layout Planner。
...
作为第一版 Full Fidelity 参考。

## 13.3 Compatible Backend
...
不得将 Compatible Render 宣称为与 PowerPoint 完全等价。
...
`Content to Editable PPT Skill` 的 Single-Slide Runtime v1.0 可以概括为：

> 在受控 Runtime 已通过 Fast Preflight 的前提下，以页面设计图作为视觉基准、以 Approved / Source Slide Content 作为文字权威源，由 Layout Planner 生成可编辑 PowerPoint 重建规格，再通过确定性资产处理、Build、Audit、实际 PowerPoint / Backend Render 和 Structural QA 建立可审核页面，最后由独立 Visual Reviewer 进行视觉检查。技术故障不得触发语义重规划，局部问题优先 Targeted Patch，只有整体理解错误才允许有限 Full Replan；所有重试必须有边界，已通过且输入未变化的阶段应复用。Reviewer 技术不可用时，在内容、可编辑性和结构 QA 均通过的条件下允许带警告降级交付，从而同时保证内容准确性、可编辑性、视觉质量和运行效率。
