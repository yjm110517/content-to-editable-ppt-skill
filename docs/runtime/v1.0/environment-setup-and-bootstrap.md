# Content to Editable PPT Skill 运行环境安装与引导规范 v1.0

> 英文名：Runtime Installation & Bootstrap Specification v1.0
...
- Python / Node 依赖隔离；
- 依赖版本锁定；
- PowerPoint / Office Backend 检测；
- Runtime Manifest；
- 首次安装验证；
- 日常快速预检；
- Runtime 损坏后的自动修复；
- Full Fidelity 与 Compatible Backend 能力分级；
- 安装失败和修复失败时的行为。
...
- 安装 Skill；
- Bootstrap Python / Node Runtime；
- 检测可用兼容 Backend；
- 运行基础 Smoke Test；
- 明确提示当前环境的 PowerPoint 保真能力。
...
- `PyYAML`
- `jsonschema`
- `pywin32`（Windows / PowerPoint Backend 需要时）
- Skill 其他正式依赖
...
# 7. Office / Rendering Backend

## 7.1 原则

PowerPoint 特定能力不应直接散落在业务流程中。

应抽象为 Runtime Backend。
...
↓
Office / Render Backend
├─ PowerPoint Backend
└─ Compatible Backend
```

## 7.2 PowerPoint Backend

第一版 Full Fidelity 参考 Backend：
...
+
PowerPoint Automation / COM
```

该 Backend 作为当前项目的最高保真验证环境。
...
## 7.3 Compatible Backend

在：

- macOS；
- Linux；
- Windows without PowerPoint；
...
> Compatible Backend 可以提供兼容能力，不承诺与 Microsoft PowerPoint Backend 完全相同的视觉保真度。
...
↓
09 Detect PowerPoint / compatible backend
↓
10 Detect key fonts / system capabilities
↓
11 Verify writable directories
↓
12 Run smoke tests
↓
13 Write runtime-manifest
↓
14 Mark READY
```
...
Windows + PowerPoint Backend 下，还应检查：
...
基本 Runtime 可用，但某些 Full Fidelity 能力不可用。
...
- 可以支持 Skill 安装；
- 可以支持部分 Content / Planning 能力；
- 可以尝试兼容 PPTX Backend；
- 不承诺与 PowerPoint Backend 相同的视觉保真度。

## 18.3 后续扩展

后续可以增加：

- 更稳定的 LibreOffice Backend；
...
Runtime Installation & Bootstrap v1.0 至少应满足以下验收条件：

1. Skill 安装后能够建立受控 Runtime；
2. Python 项目依赖不依赖用户全局 site-packages；
3. Node 项目依赖不依赖全局 npm 包；
4. 安装后能够生成 Runtime Manifest；
5. 能够检测当前 Office / Render Backend；
6. Windows + PowerPoint 环境能够识别为 Full Fidelity；
...
> Skill 应尽可能在安装或首次运行阶段自动准备、隔离并验证自身所需 Runtime，而不是依赖用户机器上任意的 Python、Node 或项目包状态。正常任务开始前必须执行快速环境预检，环境未达到 Ready 状态时不得调用 Layout Planner。可修复的运行环境问题应通过 Runtime Repair 自动恢复，并继续原任务；无法恢复时明确产生 Environment Failure。Windows + Microsoft PowerPoint 作为第一版 Full Fidelity 参考 Backend，其他环境可以使用 Compatible Backend，但必须明确其能力与视觉保真限制。
