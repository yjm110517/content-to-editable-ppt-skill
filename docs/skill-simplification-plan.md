...
本计划是已完成精简工作的唯一总记录。根据用户要求，仅建立下列三份配套实施文档；不得继续扩展第四份阶段文档、精简状态机或证据体系。

除非明确标为“后续质量工作”，以下“当前”、基线、实施步骤和验收清单均记录阶段 1 开始时的历史状态；当前正式多页行为以 ADR-043、`SKILL.md` 和 Runtime 实现为准。
...
| 3 | [删除旧体系、核心验收与文档收口](simplification/03-legacy-removal.md) | 已完成（PR #62） |
...
- [x] README 和 `SKILL.md` 只描述一个多页主路径和一个独立单页兼容路径；
- [x] 用户侧不出现 P0～P5；
- [x] 用户只需提供材料、确认方案、接收结果；
- [x] 正式 Skill 只有一个多页主入口，并完整保留独立单页兼容入口；
- [x] 正式入口只接受原始用户输入，不消费内部 Artifact；
- [x] Smoke、Fixture、Baseline、Report、Replay 和 Field Validation 从生产路径不可达；
- [x] 安装包内每个文件都有明确生产价值、核心风险价值或依赖价值；
- [x] 阶段 Gate、Evidence、状态机和 Schema 已随旧能力删除或合并；
- [x] Planner、Reviewer 配置只在真实保留入口确有需要时存在；
- [x] 依赖清单、锁文件、许可证和必要 Runtime 完整保留；
- [x] 一个真实小型多页 Deck 可以端到端生成并打开；
- [x] 正式安装包、外部入口和长期 Artifact 相比当前基线实现净减少。

以下事项是当时列出的、未随精简阶段实施的后续质量工作；它们不阻塞第一目标完成，也不构成当前 Runtime 需求：
