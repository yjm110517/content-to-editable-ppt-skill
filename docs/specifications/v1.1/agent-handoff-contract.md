# Content to Editable PPT Skill Agent 职责与交接契约 v1.1


## v1.1 变更摘要

本版本不增加新的 Agent，继续冻结为 Host Agent、Layout Planner、Visual Reviewer 三个角色；主要补充 Runtime Installation & Bootstrap 带来的环境职责边界。

主要变化：

- Runtime 仍是确定性子系统，不新增 Runtime Agent；
- Host 在需要重建能力的任务中负责发起 Runtime readiness 检查；
- Runtime 未 Ready 时，Host 必须先调度 Bootstrap / Repair，不能先调用 Layout Planner；
- Layout Planner 与 Visual Reviewer 均不得承担 Python / Node / PowerPoint 环境安装与修复；
- Builder / Runtime 契约扩展 Bootstrap、Verify、Fast Preflight、Repair 与 Backend Detection；
- 标准交接流程增加 Runtime Ready Gate。
...
- 选择或切换系统 Python 解释器；
- 检测或修复 PowerPoint COM / Office Backend；
- 因局部问题无理由重新规划整页。
...
- Fast Preflight；
- Runtime Repair；
- Office / Render Backend detection；
- Runtime Ready / Degraded / Unsupported / Failed 状态。
...
- 字体处理；
- PPTX 生成；
- PowerPoint / Compatible Backend 渲染；
- 结构 QA；
- 基础可编辑性检查；
- Patch 应用；
- 多页合并；
...
`Content to Editable PPT Skill` v1.1 的 Agent 协作原则可以概括为：
